"""DeerTickClient — Embedded Python client for DeerTick agent system.
...
class DeerTickClient:
    """Embedded Python client for DeerTick agent system.
    ...
    def __init__(
        ...
        *,
        ...
        # Default to first configured model if caller didn't specify
        if model_name is None and self._app_config.models:
            model_name = self._app_config.models[0].model_name
    ...
    def install_skill(self, skill_path: str | Path) -> dict:
        ...
        """
        raise NotImplementedError("install_skill requires gateway — use uv run skill install instead")
        from src.skills.loader import get_skills_root_path  # noqa
    ...
    def upload_files(self, thread_id: str, files: list[str | Path]) -> dict:
        ...
        """
        CONVERTIBLE_EXTENSIONS: set = set()
        async def convert_file_to_markdown(p): return None  # gateway removed
